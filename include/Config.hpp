...
class Config {
  public:
    using pic24_register = std::pair<std::string, uint16_t>; /**< Create a new "internal" type called pic24_register, as
  ...
  public:
     ...
     * @param       bool ChangeIfExists : true to change value if register is already in the ConfigList, false (default)
     * to make sure an existing register value won't be touched.
     *
     * @throw : std::invalid_argument("Register already exists.") if ChangeIfExits is false and the register is already
     * listed.
     */
     ...
     * @throw : std::invalid_argument("Register doesn't exist.") if the key doesn't exit.
     */
    void remove(const pic24_register &reg) {
        this->remove(reg.first);
    }; /**< Simple call to the std::string based remove function */
    ...
    void dump() const noexcept {
        for (auto elt : this->ConfigList)
            std::println("{0:s} : 0x{1:04X}", elt.first, elt.second);
    };
};
